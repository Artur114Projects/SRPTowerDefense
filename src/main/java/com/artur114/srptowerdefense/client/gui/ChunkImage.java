    ...
    private BufferedImage createImage() {
        ...
        for (int x = 0; x < width; x++) {
            for (int z = 0; z < height; z++) {
                ...
                int biomeC = this.biomeColorFor(this.world, pos, state);

                if (biomeC != -1) {
                    float light = ((red + green + blue) / (255.0F * 3.0F)) + 0.25F;

                    red = (int) Math.min(((biomeC >> 16) & 255) * light, 255);
                    green = (int) Math.min(((biomeC >> 8) & 255) * light, 255);
                    blue = (int) Math.min((biomeC & 255) * light, 255);
                }
